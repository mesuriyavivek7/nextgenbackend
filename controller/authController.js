...
export const loginUser = async (req, res, next) =>{
    try{
      ...
      if(!email || !password) return res.status(200).json({message:"Please provide all required fields.",success:false})
      ...
      res.cookie('token', token, {
        ...
        httpOnly: true,
        domain:
         process.env.NODE_ENV === "production" ? ".nextgen.co.in" : undefined,
         secure: process.env.NODE_ENV === "production",
         sameSite: process.env.NODE_ENV === "production" ? "none" : "lax",
      });

      return res.status(200).json({message:'Login Successfully',data:{token,userId:user.mongoid}, success:true})
...
export const validateToken = async (req, res, next) =>{
    try{
      ...
      return res.status(200).json({message:"Token validate successfully.",data:{token, userId:user.mongoid},success:true})
...
export const signupUser = async (req, res, next) =>{
    try{
        ...
        if(!full_name || !email || !password ) return res.status(400).json({message:"Please provide all required fields."})
...
export const logoutPortal = async (req, res, next) =>{
    try {
      res.clearCookie("token", {
        ...
        secure: process.env.NODE_ENV === "production",
        sameSite: process.env.NODE_ENV === "production" ? "none" : "lax",
        domain: process.env.NODE_ENV === "production" ? ".nextgen.co.in" : undefined,
      });
